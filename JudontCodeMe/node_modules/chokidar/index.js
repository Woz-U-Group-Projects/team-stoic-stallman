...
import { normalize, isAbsolute, join, dirname, basename, resolve, relative } from 'path';
...
const normalizeIgnored = (cwd = '') => (path) => {
    ...
    return normalizePathToUnix(isAbsolute(path) ? path : join(cwd, path));
};
...
const getAbsolutePath = (path, cwd) => {
    if (isAbsolute(path)) {
        ...
        return BANG + join(cwd, path.substring(1));
    } else {
    ...
    async remove(item) {
        ...
        if (!this.items.size) {
            ...
            try {
                ...
                this._removeWatcher(dirname(dir), basename(dir));
            }
        }
    }
...
class WatchHelper {
    constructor(path, watchPath, follow, fsw) {
        ...
        this.fullWatchPath = resolve(watchPath);
    ...
    entryPath(entry) {
        return join(this.watchPath,
            relative(this.watchPath, this.checkGlobSymlink(entry))
        );
    }
    ...
    getDirParts(path) {
        ...
        expandedPath.forEach((path) => {
            parts.push(relative(this.watchPath, path).split(SLASH_OR_BACK_SLASH));
        });
        return parts;
    }
...
class FSWatcher extends EventEmitter {
    ...
    async add(paths_, _origAdd, _internal) {
        ...
        if (this.options.useFsEvents && this._fsEventsHandler) {
            ...
            results.forEach((item) => {
                ...
                this.add(dirname(item), basename(_origAdd || item));
            });
        }
    ...
    getWatched() {
        ...
        this._watched.forEach((entry, dir) => {
            const key = this.options.cwd ? relative(this.options.cwd, dir) : dir;
    ...
    async _emit(event, path, val1, val2, val3) {
        ...
        if (opts.cwd) path = relative(opts.cwd, path);
        /** @type Array<any> */
    ...
    _awaitWriteFinish(path, threshold, event, awfEmit) {
        ...
        if (this.options.cwd && !isAbsolute(path)) {
            fullPath = join(this.options.cwd, path);
        }

        const now = new Date();

        const awaitWriteFinish = (prevStat) => {
            _stat(fullPath, (err, curStat) => {
    ...
    _remove(directory, item) {
        ...
        let relPath = path;
        if (this.options.cwd) relPath = relative(this.options.cwd, path);
    ...
    _closePath(path) {
        ...
        this._getWatchedDir(dir).remove(basename(path));
    }
...
const _FSWatcher = FSWatcher;
export { _FSWatcher as FSWatcher };
